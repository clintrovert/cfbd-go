// Package cfbd provides a minimal, type-safe Golang client for the College
// Football Data API.
package cfbd
...
const (
	...
	playTypeKey           = "playType"
	providerKey           = "provider"
	recruitTypeKey        = "recruitType"
	schoolKey             = "school"
	stateKey              = "state"
	athleteIDKey          = "athleteId"
	statTypeIDKey         = "statTypeId"
	team1Key              = "team1"
	team2Key              = "team2"
	downKey               = "down"
...
func (c *Client) GetPlays(
	...
	setString(values, defenseConferenceKey, request.DefenseConference)
	setString(values, conferenceKey, request.Conference)
	setString(values, playTypeKey, request.PlayType)
	setString(values, seasonTypeKey, request.SeasonType)
	setString(values, classificationKey, request.Classification)
...
func (c *Client) GetPlayStats(
	...
	setString(values, teamKey, request.Team)
	setInt32(values, gameIDKey, request.GameID)
	setInt32(values, athleteIDKey, request.AthleteID)
	setInt32(values, statTypeIDKey, request.StatTypeID)
	setString(values, seasonTypeKey, request.SeasonType)
	setString(values, conferenceKey, request.Conference)
...
func (c *Client) GetTeamMatchup(
	...
	values := url.Values{}
	setString(values, team1Key, request.Team1)
	setString(values, team2Key, request.Team2)
...
func (c *Client) SearchPlayers(
	...
	values := url.Values{}
	setString(values, searchTermKey, request.SearchTerm)
	setInt32(values, yearKey, request.Year)
	setString(values, teamKey, request.Team)
...
// GetTransferPortalPlayers retrieves player transfer portal information
// based on the provided request parameters.
//
...
func (c *Client) GetBettingLines(
	...
	setString(values, awayKey, request.Away)
	setString(values, conferenceKey, request.Conference)
	setString(values, providerKey, request.Provider)
...
// GetPlayersRecruitingRankingsRequest is the request configuration for the
// resource located at GET /recruiting/players.
...
// GetPlayerRecruitingRankings retrieves recruiting information for players
// based on the provided request parameters.
//
...
// GetTeamPositionGroupRecruitingRankingsRequest is the request configuration
// for the resource located at GET /recruiting/groups.
...
// GetTeamPositionGroupRecruitingRankings retrieves aggregated team recruiting
// information based on the provided request parameters.
//
...
func (c *Client) GetTeamPositionGroupRecruitingRankings(
	...
	setString(values, teamKey, request.Team)
	setString(values, conferenceKey, request.Conference)
	setString(values, recruitTypeKey, request.RecruitType)
...
func (c *Client) GetPredictedPoints(
	...
	v := url.Values{}
	setInt32(v, downKey, request.Down)
	setInt32(v, distanceKey, request.Distance)
...
func (c *Client) GetDraftPicks(
	...
	setInt32(values, yearKey, request.Year)
	setString(values, teamKey, request.Team)
	setString(values, schoolKey, request.School)
	setString(values, conferenceKey, request.Conference)
	setString(values, positionKey, request.Position)
...
func (c *Client) unmarshal(b []byte, out proto.Message) error {
	if out == nil {
		return ErrResponseWasEmpty
	}
	if len(bytes.TrimSpace(b)) == 0 || isJSONNull(b) {
		return ErrResponseWasNotJSON
	}

	if err := c.unmarshaller.Unmarshal(b, out); err != nil {
		return fmt.Errorf("error occurred during unmarshal; %w", err)
	}
...
func setString(v url.Values, key string, val string) {
	...
	// Explicitly URL encode the string value before setting it as a
	// query parameter.
	//
	// This ensures proper encoding of special characters in all string
	// parameters.
...
//
// nolint:unparam // key is kept for API consistency even though currently only
// one key is used
